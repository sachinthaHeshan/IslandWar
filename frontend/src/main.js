...
function updateFireButton(){const btn=document.querySelector('.touch-btn.touch-fire');if(!btn||!touch?.enabled)return;const loading=isReloadingNow();btn.classList.toggle('reloading',loading);btn.setAttribute('aria-label',loading?'Reloading':'Fire');}
