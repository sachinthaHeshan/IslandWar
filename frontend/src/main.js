...
import {
  createInventory, switchSlot, switchWeapon, addWeapon, activeWeapon, activeAmmo,
...
let gun=createGun('rifle');gun.position.set(.44,1.34,-.5);player.add(gun);
...
function equipGun(type){player.remove(gun);gun.traverse(o=>o.geometry?.dispose());gun=createGun(type);gun.position.set(.44,1.34,-.5);player.add(gun);flash.parent?.remove(flash);gun.add(flash);flash.position.set(0,.025,type==='sniper'?-1.35:-.91);}
...
let yaw=0,pitch=-.06,crawlBlend=0,active=false,complete=false,reloading=0,reloadingLeft=0,hits=0,shots=0,elapsed=0,shotCooldown=0,flashTime=0,hitTime=0,muted=false;
...
function applyStance(dt){const target=multiplayer.state.connected?(networkSelf?.crawling?1:0):(wantsCrawl()&&soloBody.grounded?1:0);crawlBlend=THREE.MathUtils.damp(crawlBlend,target,12,dt);player.scale.y=THREE.MathUtils.lerp(1,.58,crawlBlend);arms.forEach(a=>{a.rotation.x=THREE.MathUtils.lerp(-.8,-1.35,crawlBlend);});gun.position.y=THREE.MathUtils.lerp(1.34,.78,crawlBlend);document.body.classList.toggle('crawling',crawlBlend>.35);}
...
island.update(time,dt,body,lootMap,multiplayer.state.snapshot?.now||Date.now());applyStance(dt);flashTime-=dt;flash.visible=flashTime>0;hitTime-=dt;$('hitmarker').style.opacity=hitTime>0?'1':'0';$('toast').style.opacity=hitTime>0?'1':'0';for(let i=tracers.length-1;i>=0;i--){const t=tracers[i];t.life-=dt;if(t.life<=0){scene.remove(t.line);t.line.geometry.dispose();t.line.material.dispose();tracers.splice(i,1);}}updateCamera();renderer.render(scene,camera);}
...
function sendNetworkInput(){
  ...
  multiplayer.send({type:'input',x:active?(Number(keys.has('KeyD'))-Number(keys.has('KeyA'))):0,z:active?(Number(keys.has('KeyS'))-Number(keys.has('KeyW'))):0,yaw,pitch,sprint:active&&!wantsCrawl()&&(keys.has('ShiftLeft')||keys.has('ShiftRight')),crawl:active&&wantsCrawl()});
}
setInterval(sendNetworkInput,50);
function networkFrame(dt){
  if(networkSelf){player.position.lerp(new THREE.Vector3(networkSelf.x,networkSelf.y,networkSelf.z),1-Math.exp(-25*dt));player.rotation.y=yaw;gun.rotation.x=networkSelf.reloading?-.5:0;const moving=active&&['KeyW','KeyA','KeyS','KeyD'].some(k=>keys.has(k));legs.forEach((leg,i)=>leg.rotation.x=moving?Math.sin(performance.now()*.012+i*Math.PI)*.45:0);}
...
gun.userData={type:'rifle'};
updateWeaponBar(inventory);
updateCamera();frame();
