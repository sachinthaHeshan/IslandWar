...
function dismissBoot(){
  ...
  if(!html.classList.contains('booting')&&html.classList.contains('ready'))return;
...
const scopeLens = new THREE.Mesh(
  ...
  new THREE.MeshBasicMaterial({ map: scopeRT.texture, depthTest: false, depthWrite: false, toneMapped: false }),
);
...
function equipGun(type){
  ...
  gun.add(flash);
  flash.position.set(0,.025,type==='rpg'?0.35:GUN_FLASH_Z[type]??GUN_FLASH_Z.pistol);
}
function applyRpgPose(){
  ...
  }else if(crawlBlend<.2){
    arms.forEach(a=>{
      a.rotation.x=THREE.MathUtils.lerp(a.rotation.x,-.8,.14);
      a.rotation.z=THREE.MathUtils.lerp(a.rotation.z,0,.14);
    });
  }
}
...
function rpgImpactTargets(end,splash){
  const hitTarget=t=>{if(!t?.alive)return;t.alive=false;t.group.visible=false;hits++;hitTime=.65;sound(true);};
...
function canAim(id){return id==='ak47'||id==='sniper';}
function usesScopeLens(id){return id==='sniper';}
function canScope(id){return usesScopeLens(id);}
function aimFov(id){
...
function scopeLensRadiusPx(){
  return Math.min(innerHeight,innerWidth)*.29;
}
...
function adjustScopeZoom(delta){
  ...
  store[id]=THREE.MathUtils.clamp(store[id]+delta*lim.step,lim.min,lim.max);
  const el=$('scope-zoom');
  if(el&&aimBlend>.15)el.textContent=scopeMagnification().toFixed(1)+'×';
}
...
function updateInteractPrompt(){const el=$('interact-prompt');if(!el)return;const loot=nearbyLoot();const alive=!multiplayer.state.connected||(networkSelf?.health??0)>0;const show=!!(active&&alive&&loot&&canPickupWeapon(loot.weapon));el.hidden=!show;if(!show)return;const icon=el.querySelector('.interact-icon');const label=el.querySelector('.interact-label');if(touch?.enabled){icon.textContent='↗';label.textContent=`Pick up ${weaponShortName(loot.weapon)}`;}else{icon.textContent='E';label.textContent=`Pick up ${weaponShortName(loot.weapon)}`;}}
...
function isReloadingNow(){return multiplayer.state.connected?!!networkSelf?.reloading:!!reloading;}
function updateFireButton(){const btn=document.querySelector('.touch-btn.touch-fire');const reloadBtn=document.querySelector('.touch-btn[data-action=reload]');if(!touch?.enabled)return;const loading=isReloadingNow();if(btn){btn.classList.toggle('reloading',loading);btn.setAttribute('aria-label',loading?'Reloading':'Fire');}if(reloadBtn){reloadBtn.classList.toggle('reloading-active',loading);reloadBtn.setAttribute('aria-label',loading?'Reloading':'Reload');}}
function updateHUD(){if(multiplayer.state.connected){updateNetworkHUD();return;}$('ammo').textContent=reloading?'··':String(activeAmmo(inventory)).padStart(2,'0');$('hits').textContent=String(hits).padStart(2,'0');$('accuracy').textContent=shots?Math.round(hits/shots*100)+'%':'—';$('timer').textContent=String(Math.floor(elapsed/60)).padStart(2,'0')+':'+String(Math.floor(elapsed%60)).padStart(2,'0');updateWeaponBar(inventory,reloading);updateFireButton();}
...
function updateCamera(){
  ...
  adsPos.copy(pivot).addScaledVector(forward,.1).addScaledVector(right,.07).add(new THREE.Vector3(0,.05,0));
  lookTarget.copy(pivot).addScaledVector(forward,300);
  const focusBlend=id==='ak47'?aimBlend:0;
  camera.position.copy(hipPos).lerp(adsPos,focusBlend*.4);
...
function renderFrame(){
  ...
  if(aimBlend<=.05||!usesScopeLens(activeWeaponId()))return;
  syncScopeCamera();
...
const alive=!multiplayer.state.connected||(networkSelf?.health??0)>0;const canPickup=id=>active&&alive&&canPickupWeapon(id);island.update(time,dt,body,lootMap,multiplayer.state.snapshot?.now||Date.now(),{canPickup,useCenterPrompt:active});updateInteractPrompt();applyStance(dt);applyRpgPose();applyAim(dt);fx.update(dt);flashTime-=dt;flash.visible=flashTime>0&&!wantsAim()&&activeWeaponId()!=='rpg';hitTime-=dt;$('hitmarker').classList.toggle('show',hitTime>0);$('toast').style.opacity=hitTime>0?'1':'0';for(let i=tracers.length-1;i>=0;i--){const t=tracers[i];t.life-=dt;if(t.life<=0){scene.remove(t.line);t.line.geometry.dispose();t.line.material.dispose();tracers.splice(i,1);}}updateCamera();renderFrame();}
...
const multiplayer = createMultiplayer({
  onSnapshot(snapshot, previous, selfID) {
    ...
    for(const event of snapshot.events||[]){
      ...
      const start=new THREE.Vector3(event.start.x,event.start.y,event.start.z);
      const end=new THREE.Vector3(event.end.x,event.end.y,event.end.z);
      if(event.weapon==='rpg'){
        fx.fireRocket({start,end,splash:8});
      }else{
        const line=new THREE.Line(new THREE.BufferGeometry().setFromPoints([start,end]),new THREE.LineBasicMaterial({color:event.shooter===selfID?'#ffe6a4':'#ffa17c',transparent:true,opacity:.9}));scene.add(line);tracers.push({line,life:.10});
      }
